...
function removeFirstOccurrences(str, value) {
  ...
  if (ind !== -1) {
    return str.slice(0, ind) + str.slice(value.length + ind);
  }
  return str;
}
...
function isPalindrome(str) {
  const latin = str.replace(/[^a-zA-Z]/g, '').toLowerCase();
  const toMid = Math.floor(latin.length / 2);
  const fromMid = latin.length % 2 ? toMid + 1 : toMid;
  return (
    latin.slice(0, toMid) === latin.slice(fromMid).split('').reverse().join('')
  );
}
...
function reverseWords(str) {
  const arr = str.split(' ');
  const outArr = arr.map((x) => x.split('').reverse().join(''));
  return outArr.join(' ');
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < arr.length; i++) {
    const ec = arr[i].charCodeAt();
    if (ec < 91 && ec > 64) arr[i] = arr[i].toLowerCase();
    else if (ec < 123 && ec > 96) arr[i] = arr[i].toUpperCase();
  }
  return arr.join('');
}
...
function encodeToRot13(str) {
  ...
  for (let e of str) {
    ...
    if (e >= 65 && e <= 90) {
      e < 78
        ? (answer += String.fromCharCode(e + 13))
        : (answer += String.fromCharCode(e - 13));
    } else if (e >= 97 && e <= 122) {
      e < 110
        ? (answer += String.fromCharCode(e + 13))
...
function getCardId(value) {
  ...
  return (
    name.indexOf(value.slice(0, -1)) +
    suit.indexOf(value[value.length - 1]) * 13
  );
}
